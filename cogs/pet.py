...
class Pet(commands.Cog):
    ...
    async def pet(self, inter: disnake.ApplicationCommandInteraction, user: disnake.User = None):
        if user is None:
            user = inter.author
        if not user.avatar:
            await inter.response.send_message(Messages.pet_unsupported_avatar)
            return
    ...
    async def pet_error(self, inter: disnake.ApplicationCommandInteraction, error):
        if isinstance(error, commands.MemberNotFound):
            await inter.response.send_message(Messages.member_not_found.format(user=inter.author.id))
            return True
