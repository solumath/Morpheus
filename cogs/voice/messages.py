...
class VoiceMess(GlobalMessages):
    ...
    playlist_group_brief = "Playlist commands"
    play_brief = "Play a song with the given query (can be url)"
    playlist_play = "Play your or friend's playlist."
    ...
    stop_brief = "Stop the player and leave the voice channel"
    volume_brief = "Change the volume of the player"
    move_brief = "Move a song in the queue to a specific place in the queue"
    bot_not_connected = "Bot is not connected to voice."
    not_in_channel = "To use bot you must be in the same voice channel."
    ...
    playlist_added_queue = "[Added the playlist **`{tracks}`** ({added} songs) to the queue.]({url})"
    track_added_queue = "[Added **`{track}`** to the queue.]({url})"
    no_track_found = "No tracks found"
    track_not_found = "{user} - Could not find any tracks with that query. Please try again."
    ...
    fetching_queue = "Fetching the queue..."
    empty_queue = "The queue is empty."
    playlist_exists = "Playlist with name `{name}` already exists."
    playlist_added = "Playlist `{name}` has been saved\n- {url}"
    no_playlist_found = "No playlists found"
    playlist_not_found = "Given playlist was not found."
    ...
    playlist_param = "Use autocomplete or input ID of playlist"
